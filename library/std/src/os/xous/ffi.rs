...
use xous::{CID, MemoryRange, SID, TID};
#[stable(feature = "rust1", since = "1.0.0")]
pub(crate) use xous::{Error, MemoryFlags};
...
fn lend_mut_impl(
    ...
    match result {
        xous::Result::MemoryReturned(_, arg1, arg2) => Ok((
...
fn connect_impl(address: ServerAddress, blocking: bool) -> Result<Connection, Error> {
    Ok(if blocking { xous::connect(address.0)? } else { xous::try_connect(address.0)? })
}
...
pub(crate) unsafe fn map_memory<T>(
    ...
    let slice = unsafe { core::slice::from_raw_parts_mut(range.as_mut_ptr() as *mut T, count) };
    Ok(slice)
}
...
pub(crate) unsafe fn unmap_memory<T>(range: *mut [T]) -> Result<(), Error> {
    ...
    let range = unsafe { xous::MemoryRange::new(addr, len)? };
...
pub(crate) fn create_thread(
    ...
    let thread_init =
        xous::arch::args_to_thread(start as usize, stack_addr, stack_size, arg0, arg1, arg2, arg3)?;
